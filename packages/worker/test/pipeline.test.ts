...
describe('pipeline', () => {
  ...
  it("sıcak yol: 'latest' modunda hedef birincil sinyalden gelir — getBlock hiç çağrılmaz", async () => {
    ...
    const fake = {
      getBlock: async () => {
        calls.getBlock += 1;
        throw new Error('sıcak yolda getBlock olmamalı');
      },
      getLogs: async () => [],
    ...
    expect(await getCursor(pool, 'idx_hot')).toBe(3n);
  });
